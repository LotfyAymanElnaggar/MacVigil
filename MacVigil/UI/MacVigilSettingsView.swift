...
struct MacVigilSettingsView: View {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    VStack(alignment: .leading, spacing: 3) {
                        Text(selection.rawValue).font(.title2.weight(.bold))
    ...
    private var sidebar: some View {
        VStack(alignment: .leading, spacing: 7) {
            HStack(spacing: 10) {
                ZStack {
                    RoundedRectangle(cornerRadius: 11).fill(LinearGradient(colors: [.blue, .purple], startPoint: .topLeading, endPoint: .bottomTrailing))
                    Image(systemName: "bolt.shield.fill").foregroundStyle(.white).font(.system(size: 21, weight: .semibold))
                }
            ...
            MVGlassCard {
                HStack {
                    Circle().fill(manager.isActive ? Color.green : Color.secondary).frame(width: 8, height: 8)
                    VStack(alignment: .leading, spacing: 1) {
                        Text(manager.isActive ? "Vigil active" : "Vigil idle").font(.caption.weight(.semibold))
                        Text(manager.isActive ? manager.configurationName : "Normal sleep behavior").font(.caption2).foregroundStyle(.secondary)
                    }
                }
            }
        }
    ...
    private var general: some View {
        VStack(spacing: 14) {
            MVGlassCard {
                VStack(spacing: 0) {
                    toggleRow("Launch at login", "Start MacVigil automatically after sign-in.", "power", updater.launchAtLoginEnabled) { updater.setLaunchAtLogin($0) }
                    ...
                    toggleRow("Automatic update checks", "Check GitHub in the background while MacVigil is running.", "bell", updater.automaticChecksEnabled) {
                        updater.automaticChecksEnabled = $0; updater.savePreferences(); updater.startPeriodicChecks()
                    }
                    Divider().padding(.leading, 42)
                    toggleRow("Auto-update when Vigil is inactive", "Install verified updates only when no Vigil session is protecting work.", "arrow.down.circle", updater.automaticInstallEnabled) {
                        updater.automaticInstallEnabled = $0; updater.savePreferences()
                    }
                }
            }
            modeDurationCard
    ...
    private var modeDurationCard: some View {
        MVGlassCard {
            VStack(alignment: .leading, spacing: 14) {
                Text("Mode & duration").font(.headline)
                HStack(spacing: 8) {
                    MVModeButton(profile: .computeGuard, selected: matches(.computeGuard)) { Task { _ = await manager.changeModeLive(.computeGuard) } }
                    MVModeButton(profile: .closedLidEco, selected: matches(.closedLidEco)) { Task { _ = await manager.changeModeLive(.closedLidEco) } }
                    MVModeButton(profile: .fullAwake, selected: matches(.fullAwake)) { Task { _ = await manager.changeModeLive(.fullAwake) } }
                }
                HStack(spacing: 6) {
                ...
                Text("Custom duration: \(formatMinutes(Int(durationMinutes.rounded())))").font(.caption).foregroundStyle(.secondary)
            }
        }
    }
        ...
        MVGlassCard {
            VStack(alignment: .leading, spacing: 8) {
                HStack {
                    ...
                    Spacer()
                    Text("\(Int(batteryReserve))%").font(.headline.monospacedDigit()).foregroundStyle(Color.accentColor)
                }
                Slider(value: $batteryReserve, in: 5...30, step: 1) { editing in
                    guard !editing else { return }
                    manager.lowBatteryCutoff = Int(batteryReserve.rounded()); manager.savePreferences()
                }
                Text("Battery safety stops Vigil at the configured reserve when enabled.").font(.caption).foregroundStyle(.secondary)
            }
        }
    }
    ...
    private var jobGuard: some View {
        VStack(spacing: 14) {
            MVGlassCard {
                HStack(spacing: 14) {
                    Image(systemName: "briefcase.fill").font(.system(size: 28)).foregroundStyle(Color.accentColor)
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Multi-job Job Guard").font(.headline)
                        Text(jobs.isWatching ? "\(jobs.activeJobCount) protected job\(jobs.activeJobCount == 1 ? "" : "s") attached." : "Protect multiple processes and commands; Vigil releases after the final selected job finishes.")
                            .font(.subheadline).foregroundStyle(.secondary)
                    }
                    Spacer()
                    Button(jobs.isWatching ? "Manage Jobs" : "Open Job Guard") {
                        NSApplication.shared.activate(ignoringOtherApps: true); openWindow(id: "job-guard")
                    }
            ...
            MVGlassCard {
                VStack(alignment: .leading, spacing: 7) {
                    ...
                    Text("Changing Compute Guard, Closed-Lid Eco, Full Awake, or individual protection options changes only the protection profile. Job Guard stays attached and continues to own when the session ends.")
                        .font(.subheadline).foregroundStyle(.secondary)
                }
            }
        }
        ...
        VStack(spacing: 14) {
            ...
            MVGlassCard {
                HStack {
                    ...
                    Spacer()
                    Button(updater.isChecking ? "Checking…" : "Check Now") { Task { await updater.checkForUpdates(userInitiated: true) } }
                        .buttonStyle(.bordered).disabled(updater.isChecking || updater.isInstalling)
                    if updater.hasUpdate {
                        Button(updater.isInstalling ? "Updating…" : "Update Now") {
                            if manager.isActive { openWindow(id: "update-confirmation") }
                            else { Task { await updater.installAvailableUpdate() } }
                        }
                        .buttonStyle(.borderedProminent).disabled(updater.isInstalling)
                    }
                }
            }
        }
                ...
                VStack(alignment: .leading, spacing: 10) {
                    HStack {
                        ...
                        if manager.authorizationInstalled {
                            Button("Remove") { Task { await manager.removeClosedLidAuthorization() } }.buttonStyle(.bordered)
                        } else {
                            Button("Install Authorization") { Task { await manager.installClosedLidAuthorization() } }.buttonStyle(.borderedProminent)
                        }
                    }
    ...
    private var appearance: some View {
        MVGlassCard {
            VStack(alignment: .leading, spacing: 12) {
                Label("Liquid Glass interface", systemImage: "drop.fill").font(.headline).foregroundStyle(Color.accentColor)
                Text("MacVigil uses native translucent materials, layered glass cards, large hit targets, clear selected states, quick-duration pills, and slider-based controls. It follows your macOS light or dark appearance automatically.")
                    .font(.subheadline).foregroundStyle(.secondary)
                Divider()
                HStack { Text("System appearance"); Spacer(); Text("Follows macOS").foregroundStyle(.secondary) }.font(.subheadline)
            }
        }
    }
    ...
    private var about: some View {
        VStack(spacing: 14) {
            MVGlassCard {
                HStack(spacing: 14) {
                    ZStack {
                        RoundedRectangle(cornerRadius: 16).fill(LinearGradient(colors: [.blue, .purple], startPoint: .topLeading, endPoint: .bottomTrailing))
                        Image(systemName: "bolt.shield.fill").font(.system(size: 34, weight: .semibold)).foregroundStyle(.white)
                    }
            ...
            MVGlassCard {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    Text("MacVigil is currently distributed with ad-hoc signing rather than Developer ID notarization.").font(.caption).foregroundStyle(.secondary)
                }
            }
        }
    }
    ...
    private var subtitle: String {
        switch selection {
        case .general: return "Everyday behavior, mode, duration, and battery reserve."
        case .vigil: return "Fine-grained protection controls, including live changes."
        ...
        case .appearance: return "Native glass presentation and system appearance."
    ...
    private func optionRow(_ title: String, _ detail: String, _ option: VigilOption) -> some View {
        HStack {
            VStack(alignment: .leading, spacing: 2) { Text(title).font(.subheadline.weight(.semibold)); Text(detail).font(.caption).foregroundStyle(.secondary) }
            Spacer()
            Toggle("", isOn: Binding(get: { optionValue(option) }, set: { enabled in Task { _ = await manager.changeOptionLive(option, to: enabled) } }))
    ...
    private func durationButton(_ title: String, _ duration: SessionDuration, _ custom: Int? = nil) -> some View {
        let selected = duration == .custom && custom != nil ? manager.selectedDuration == .custom && manager.customMinutes == custom : manager.selectedDuration == duration
        return Button(title) { if let custom { durationMinutes = Double(custom) }; Task { _ = await manager.changeDurationLive(duration, customMinutes: custom) } }
    ...
    private func matches(_ profile: RuntimeProfile) -> Bool {
        switch profile {
        case .computeGuard: return manager.preventSystemSleep && manager.preventIdleSystemSleep && !manager.keepDisplayAwake && manager.vetoIdleSleepRequests && !manager.useGlobalSleepDisable && !manager.useKernelLidGuard && !manager.darkenBuiltinDisplayOnLidClose
        case .closedLidEco: return manager.preventSystemSleep && manager.preventIdleSystemSleep && manager.vetoIdleSleepRequests && manager.useGlobalSleepDisable && manager.useKernelLidGuard && manager.darkenBuiltinDisplayOnLidClose
        case .fullAwake: return manager.preventSystemSleep && manager.preventIdleSystemSleep && manager.keepDisplayAwake && manager.vetoIdleSleepRequests && !manager.useGlobalSleepDisable && !manager.useKernelLidGuard && !manager.darkenBuiltinDisplayOnLidClose
        }
    }
